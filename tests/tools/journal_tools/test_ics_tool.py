...
class TestTaskToVeventLines(unittest.TestCase):
    ...
    def test_status_todo(self):
        props = self._vevent(title='T', status='todo')
        self.assertEqual(props['STATUS'], 'TENTATIVE')

    def test_status_done(self):
        props = self._vevent(title='T', status='done')
        self.assertEqual(props['STATUS'], 'CONFIRMED')

    def test_status_in_progress(self):
        props = self._vevent(title='T', status='in progress')
        self.assertEqual(props['STATUS'], 'CONFIRMED')

    def test_status_started(self):
        props = self._vevent(title='T', status='started')
        self.assertEqual(props['STATUS'], 'CONFIRMED')

    def test_status_failed(self):
        props = self._vevent(title='T', status='failed')
        self.assertEqual(props['STATUS'], 'CANCELLED')

    def test_status_unknown_defaults_to_tentative(self):
        props = self._vevent(title='T', status=None)
        self.assertEqual(props['STATUS'], 'TENTATIVE')

    def test_priority_high(self):
        props = self._vevent(title='T', priority='!!!')
    ...
    def test_priority_low(self):
        props = self._vevent(title='T', priority='!')
        self.assertEqual(props['PRIORITY'], '9')

    def test_priority_none(self):
        props = self._vevent(title='T')
