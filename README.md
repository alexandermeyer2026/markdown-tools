...
**If everything is stored in markdown files, the functionality is limited by creativity.**

Markdown is the lingua franca of knowledge management. It's human-readable, version-controllable, and compatible with virtually every tool and platform. By building tools that operate on markdown files, we create solutions that are:
...
The project includes various tools for managing markdown-based journals and tasks. Each tool is designed to be independent and can be used on its own or in combination with others.

For detailed documentation on individual tools, see their respective README files (coming soon).
...
└── tools/                  # Tool implementations
    └── journal_tools/      # Journal tools (timeline, catch-up)
```
...
We envision a future where knowledge and task management isn't constrained by software limitations, but empowered by the simplicity and universality of markdown. Every tool you need, working with files you control, in formats that will outlast any software vendor.

Join us in building this ecosystem.
